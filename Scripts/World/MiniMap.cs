...
public partial class MiniMap : Control
{
    ...
    public override void _Draw()
    {
        ...
        foreach (var hallway in Hallways)
        {
            ...
            // Show hallway if either endpoint is the current room, or both are discovered
            bool eitherIsCurrent = fromRoom.Position == PlayerCurrentRoom || toRoom.Position == PlayerCurrentRoom;
            bool bothDiscovered = fromRoom.IsDiscovered && toRoom.IsDiscovered;
            if (!eitherIsCurrent && !bothDiscovered) continue;
        ...
        {
            // Only show discovered rooms (or the current room, in case discovery marking lags by a frame)
            if (!room.IsDiscovered && room.Position != PlayerCurrentRoom) continue;
            ...
            // Checkmark will be drawn on cleared rooms.
